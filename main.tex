...
\subsection{... mais un fonctionnement différent}
...
Ces frameworks possédent deux composants : les schedulers et les executors. Ces deux composants sont situés respectivement au niveau du control-plane et sur les worker nodes. Cela vient donc complexifier l'architecture d'un cluster Mesos.\\
...
Il arrive que certaines applications soit déployé dans des environnements différents (par exemple dev, qf, prod,etc...). Plutôt que d'écrire plusieurs fois le même fichier en modifant uniquement certains paramêtre il peut être intéressant de bénéficier d'un squelette de l'application.
...
Universe n'est pas directement un gestionnaire de paquet mais un format de templating développé afin d'être utilisé par le gestionnaire de paquets DC/OS. Ce gestionnaire de paquets est accessible depuis l'interface graphique de DC/OS ou depuis la CLI DC/OS. Par défaut DC/OS propose 120 applications installables par le biais de ce gestionnaire de paquets. Pour plus d'informations sur le fonctionnement des universes le lecteur peut se rendre en annexe \ref{Universe} \newline 

Helm, quant à lui, est une CLI qui permet à la fois de déployer des paquets (appelé charts) dans Kubernetes mais aussi de lister les services déjà installés, de faire des montées de versions, de créer des paquets, d'ajouter de nouveaux répertoires contenant d'autre paquets. Et, comme Universe, c'est aussi un outil qui fournit son propre type de templating.\newline
